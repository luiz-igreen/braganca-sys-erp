...
def carregar_colaboradores_premios(_engine):
    return pd.read_sql("SELECT id, nome, cargo, admissao, demissao, data_afastamento, data_retorno, salario_mes_12_24, situacao FROM cadastro_geral_colaborador ORDER BY nome ASC", _engine)

@st.cache_data(ttl=30)
def carregar_premios_lancados(_engine, id_colab, comp):
    return pd.read_sql(text("SELECT tipo_lancamento, valor_lancamento, status_pagamento FROM historico_premiacoes_e_folha WHERE id_colaborador = :id_c AND competencia = :comp"), _engine, params={"id_c": id_colab, "comp": comp})
...
def render(engine, format_brl_number, format_currency_brl, clean_money_to_db, injetar_autofoco, LISTA_SERVICOS_PREMIO):
    ...
    with st.spinner("⏳ Carregando colaboradores elegíveis..."):
        try:
            df_colabs = carregar_colaboradores_premios(engine)
            ...
            if not colabs_elegiveis:
                ...
                with aba_lote:
                    ...
                    if c_btn_lt1.button("💾 Salvar Lote Inteiro", type="primary"):
                        with st.spinner("⏳ Salvando lançamentos em lote..."):
                ...
                with aba_ind:
                    ...
                    with st.spinner("⏳ Carregando prêmios já lançados..."):
                        df_ja_lancado = carregar_premios_lancados(engine, dados_c['id'], comp_sel)
                        if not df_ja_lancado.empty:
                            ...
                            st.info("Nenhum prêmio lançado para este colaborador nesta competência.")
                    ...
                    if st.session_state.get('zaut_acao') != 'lancando':
                        ...
                        if c_btn_i1.button("💾 Gravar", type="primary", key="btn_ind_gravar"):
                            with st.spinner("⏳ Gravando prêmio individual..."):
